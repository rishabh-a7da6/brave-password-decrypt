...
if __name__ == '__main__':
    try:
        with open('passwords.csv', mode='w', newline='', encoding='utf-8') as decrypted_file:
            ...
            # getting Secret key
            secret = get_brave_secret_key()

            # extracting all profiles location in a list
            folders = [element for element in os.listdir(BRAVE_PATH) if element.startswith("Profile") or element == "Default"]

            # iterating over each folder for Login Data file
            for folder in folders:
                brave_db = os.path.normpath(r"%s\%s\Login Data"%(BRAVE_PATH,folder))

                # saving Login Data file in a sqlite db and getting a connection
                connection = get_db_connection(brave_db)
